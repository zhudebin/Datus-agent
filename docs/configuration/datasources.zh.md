...
> **迁移说明**：旧的 `namespace:` 配置格式会在运行时自动迁移到 `services.datasources`。此外，早期版本里的 `services.databases` 键已重命名为 `services.datasources`；运行 `python -m datus.configuration.config_migrator --config conf/agent.yml` 可自动改写 YAML，或者手动改名——运行时会拒绝旧键。
